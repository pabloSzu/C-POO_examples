...
Visual::Visual() : Ogl(), x(0), y(0), z(0), rotacion_hacia_laterales(0), rotacion_arriba_abajo(0), lastPos(0,0), horizontal_sens(1.1), vertical_sens(0.7),
                   mouse_x(0), mouse_y(0)  {
...
void Visual::mouseMoveEvent(QMouseEvent *e) {
    ...
    if(horizontal_aux > 10)
        horizontal_aux = 10;
    else if (horizontal_aux < -10)
        horizontal_aux = -10;
...
void Visual::paintGL()  {
    ...
    if(horizontal_aux > 0){
        ...
        horizontal_aux--;
        rotacion_hacia_laterales += horizontal_sens;

        if(rotacion_hacia_laterales > 360)
            rotacion_hacia_laterales = 0;
    }else if (horizontal_aux < 0){
        horizontal_aux++;
        rotacion_hacia_laterales -= horizontal_sens;

        if(rotacion_hacia_laterales < 0)
            rotacion_hacia_laterales = 360;
    }

    if(vertical_aux > 0){

        vertical_aux--;
        rotacion_arriba_abajo += vertical_sens;

        if(rotacion_arriba_abajo > 40)
            rotacion_arriba_abajo = 40;
    }else if (vertical_aux < 0){
        vertical_aux++;
        rotacion_arriba_abajo -= vertical_sens;
...
void Visual::cargarTexturas()   {
    ...
    configurarTextura(idTexturas[0], "../../../../ejercicio_39/Texturas/techo.jpg", textura[0]);
    configurarTextura(idTexturas[1], "../../../../ejercicio_39/Texturas/pared.png", textura[2]);
    configurarTextura(idTexturas[2], "../../../../ejercicio_39/Texturas/piso.jpg", textura[2]);
}
